-- Seed script: game-lists app (games embedded in game_list_games)
-- Drops existing schema, creates tables, seeds users, lists, games, follows, likes, comments
...
-- Game list games (embedded game info)
...
-- Insert 12 realistic users
INSERT INTO users (username, profile_image) VALUES
('PixelPioneer', 'https://i.pravatar.cc/150?img=1'),
('RetroRaven', 'https://i.pravatar.cc/150?img=2'),
('GameGuru', 'https://i.pravatar.cc/150?img=3'),
('IndieNinja', 'https://i.pravatar.cc/150?img=4'),
('ArcadeQueen', 'https://i.pravatar.cc/150?img=5'),
('QuestSeeker', 'https://i.pravatar.cc/150?img=6'),
('LootLlama', 'https://i.pravatar.cc/150?img=7'),
('RogueByte', 'https://i.pravatar.cc/150?img=8'),
('XPCollector', 'https://i.pravatar.cc/150?img=9'),
('BossBattler', 'https://i.pravatar.cc/150?img=10'),
('SpeedRunner', 'https://i.pravatar.cc/150?img=11'),
('CozyGamer', 'https://i.pravatar.cc/150?img=12');
...
-- Seeding logic (lists, games per list, follows, likes, comments).
-- This procedural block:
--  * creates 4..12 lists per user,
--  * 4..24 unique games per list (sampled from the seed_games above),
--  * each user follows 0..6 other users,
--  * each list receives 3..40 likes but capped to total user count (<=12 here),
--  * each list receives 2..12 comments.
DO $$
DECLARE
  adjs TEXT[] := ARRAY[
...
BEGIN
  ...
  FOR u IN SELECT id FROM users ORDER BY id LOOP
    pinned_idx := 1;
    list_count := (floor(random()*9)::int + 4); -- 4 to 12
    FOR i IN 1..list_count LOOP
      ...
      game_count := (floor(random()*21)::int + 4); -- 4 to 24
      pos := 1;
  ...
  FOR u IN SELECT id FROM users ORDER BY id LOOP
    follow_count := (floor(random()*7))::int; -- 0..6
  ...
  FOR l IN SELECT id, user_id FROM game_lists LOOP
    num_likes := (floor(random()*38)::int + 3); -- desired 3..40
  ...
  FOR l IN SELECT id FROM game_lists LOOP
    num_comments := (floor(random()*11)::int + 2); -- 2..12
